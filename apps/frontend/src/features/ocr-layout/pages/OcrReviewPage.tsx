...
export function OcrReviewPage() {
  ...
  const pages = useDocumentPages(document); const [page, setPage] = useState(1); const review = useReviewState(document.parsed?.evidence ?? []); const selectedPage = pages.find((item) => item.number === page) ?? pages[0];
  ...
  return <div className="docdai-ocr-review"><header className="docdai-review-heading"><div><span className="docdai-eyebrow">Revisión humana requerida</span><h1>{document.filename}</h1><p>OCR original → parser → corrección humana. Ningún dato se aprueba automáticamente.</p></div><Link className="btn btn-outline-secondary" to="/doctor/documents">Volver a documentos</Link></header><div className="docdai-review-grid"><PageThumbnails pages={pages} selected={page} onSelect={setPage} /><DocumentViewer page={selectedPage} totalPages={pages.length} filename={document.filename} /><ExtractionPanel fields={review.fields} selectedId={review.selectedId} onSelect={review.setSelectedId} /></div><EvidencePanel field={review.selected} onUpdate={(patch) => review.selected && review.update(review.selected.id, patch)} />{document.ocr ? <details className="docdai-ocr-details"><summary>Texto OCR de referencia</summary><pre>{document.ocr.text}</pre></details> : null}<ReviewActions pending={review.pending} onSave={save} onApprove={approve} /></div>;
}
