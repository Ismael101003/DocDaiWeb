...
export function useDocumentPages(document: ProcessingDocument): DocumentPage[] {
  return useMemo(() => Array.from({ length: document.pages ?? 1 }, (_, index) => {
  ...
  }), [document.pages, document.preparedImages, document.previewUrl]);
}
